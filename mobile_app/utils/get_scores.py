...
class GetScores():
    def __init__(self, model_dir="runs\\train180\\weights\\best.pt"):
        ...
        self.scoring_radii = np.array([0, 6.35, 15.9, 107.4 - ring, 107.4, 170.0 - ring,
                                       170.0])  # inside radius of the corresponding regions in scoring_names
        ...
        self.segment_angles = np.array(
            [-9, 9, 27, 45, 63, -81, -63, -45, -27])  # minimum angle for corresponding pairs of numbered segments below
    ...
    def process_yolo_output(self, output):
        ...
        for i in range(len(classes)):
            if classes[i] == 4 and len(dart_coords) < 3:
                ...
                if np.all(calibration_coords[
                              calibration_i] == -1):  # don't overwrite if respective calibration point already detected
    ...
    def find_homography(self, calibration_coords, image_shape):
        ...
        H_matrix = cv2.findHomography(calibration_coords[mask] * image_shape,
        ...
        for i in range(len(transformed_darts)):
            ...
            if abs(angles[i]) >= 81:
                ...
                possible_numbers = self.segment_numbers[
                    np.where(self.segment_angles == max(self.segment_angles[self.segment_angles <= angles[i]]))][0]
